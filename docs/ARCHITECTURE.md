...
flowchart TB
    subgraph App["SMS Spam Killer Application"]
        Provider["iMessage Provider<br/>(SQLite Reader)"]
        Classifiers["Classifiers<br/>(Rules + AI)"]
        Actions["Actions<br/>(Delete, Notify)"]
        UserPlugins["User Plugins<br/>(Custom)"]
    end

    subgraph Engine["@tagrouter/engine"]
        Contracts["Contracts<br/>(Interfaces)"]
        Core["TagRouterEngine<br/>(Orchestrator)"]
        EventBus["EventBus<br/>(Observability)"]
        PluginLoader["PluginLoader<br/>(YAML + Code)"]
    end
...
- [tagrouter-complete-updated-design.md](tagrouter-complete-updated-design.md)
- [tagrouter-high-level-design.md](tagrouter-high-level-design.md)
